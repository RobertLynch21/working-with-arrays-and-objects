...
const taxRate = 0.07
let postTaxPrices = prices.map(price => (price + price *taxRate));
...
//Code Here
let totalPopulation = populations.reduce((acc, pop)  => acc + pop, 0)
...
let bobsorders  = purchases.filter(purchase => purchase.owner === "Bob")

let bobsTotal = bobsorders.reduce((total, purchase) => total + purchase.price, 0)
